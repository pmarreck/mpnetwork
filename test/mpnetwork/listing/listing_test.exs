defmodule Mpnetwork.ListingTest do
  ...
  describe "attachments" do
    ...
    @test_attachment_binary_data "iVBORw0KGgoAAAANSUhEUgAAAAUAAAAFCAYAAACNbyblAAAAHElEQVQI12P4//8/w38GIAXDIBKE0DHxgljNBAAO9TXL0Y4OHwAAAABJRU5ErkJggg=="
                                 |> Base.decode64!()
    @test_attachment_binary_meta Upload.extract_meta_from_binary_data(
                                   @test_attachment_binary_data,
                                   "image"
                                 )
    @test_attachment_sha256_hash Upload.sha256_hash(@test_attachment_binary_data)
    # supposedly a gif
    @test_attachment_new_binary_data "R0lGODlhAQABAIAAAAAAAP///yH5BAEAAAAALAAAAAABAAEAAAIBRAA7"
                                     |> Base.decode64!()
    @test_attachment_new_binary_meta Upload.extract_meta_from_binary_data(
                                       @test_attachment_new_binary_data,
                                       "image"
                                     )
    ...
    @listing_create_attrs %{
      ...
      block_num: "1",
      lot_num: "A",
      live_at: ~N[2010-04-17 12:00:00],
      expires_on: ~D[2010-05-17],
      state: "NY",
    ...
    @attachment_update_attrs Enum.into(
                               %{data: @test_attachment_new_binary_data},
                               @post_update_attrs
                             )
    ...
    def fixture(:listing, user \\ user_fixture()) do
      {:ok, listing} =
        Realtor.create_listing(
          Enum.into(%{user_id: user.id, broker_id: user.office_id}, @listing_create_attrs)
        )
