defmodule Map.Helpers do
  ...
  def underscore_keys(map = %{}) do
    map
    |> Enum.map(fn {k, v} -> {(k |> Macro.underscore) |> String.replace("-", "_"), underscore_keys(v)} end)
    |> Enum.into(%{})
  end

  # Walk the list and atomize the keys
  # of any map members
  ...
  # Converts strings to existing atoms, returns key untouched if errors
  def safe_to_atom(k) do
    try do
