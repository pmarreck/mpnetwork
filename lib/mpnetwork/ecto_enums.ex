defmodule Mpnetwork.EnumMaps do
  ...
  def map_class_type_int_to_ext(k) when is_binary(k),
    do: map_class_type_int_to_ext(String.to_existing_atom(k))
  ...
  @listing_status_type_opts Enum.zip([" " | @listing_status_types_ext], [
                              nil | @listing_status_types_int
                            ])
  ...
  def listing_status_types_for_priority_search,
    do:
      Enum.zip(
        @listing_status_types_int,
        Enum.map(@listing_status_types_int, fn w -> "#{w} lst/#{w}" end)
      )

  def listing_status_types_for_search,
    do: Enum.zip(@listing_status_types_int, @listing_status_types_ext_alpha_only)

  @listing_status_types_int_to_ext_map Enum.zip(
                                         @listing_status_types_int,
                                         @listing_status_types_ext
                                       )
                                       |> Map.new()
  @listing_status_types_ext_to_int_map Enum.zip(
                                         @listing_status_types_ext,
                                         @listing_status_types_int
                                       )
                                       |> Map.new()

  def map_listing_status_type_int_to_ext(k) when is_atom(k),
    do: @listing_status_types_int_to_ext_map[k]

  def map_listing_status_type_int_to_ext(k) when is_binary(k),
    do: map_listing_status_type_int_to_ext(String.to_existing_atom(k))

  def map_listing_status_type_ext_to_int(k) when is_binary(k),
    do: @listing_status_types_ext_to_int_map[k]
  ...
  def map_basement_type_int_to_ext(k) when is_binary(k),
    do: map_basement_type_int_to_ext(String.to_existing_atom(k))

  def map_basement_type_ext_to_int(k) when is_binary(k), do: @basement_types_ext_to_int_map[k]

  # Waterfront Types
  @waterfront_types_ext [
    "Bay",
    "Canal",
    "Creek",
    "Harbor",
  ...
  @waterfront_types_int_to_ext_map Enum.zip(@waterfront_types_int, @waterfront_types_ext)
                                   |> Map.new()
  @waterfront_types_ext_to_int_map Enum.zip(@waterfront_types_ext, @waterfront_types_int)
                                   |> Map.new()

  def map_waterfront_type_int_to_ext(k) when is_atom(k), do: @waterfront_types_int_to_ext_map[k]

  def map_waterfront_type_int_to_ext(k) when is_binary(k),
    do: map_waterfront_type_int_to_ext(String.to_existing_atom(k))
  ...
  @front_exposure_type_opts Enum.zip([" " | @front_exposure_types_ext], [
                              nil | @front_exposure_types_int
                            ])
  def front_exposure_type_select_opts, do: @front_exposure_type_opts

  @front_exposure_types_int_to_ext_map Enum.zip(
                                         @front_exposure_types_int,
                                         @front_exposure_types_ext
                                       )
                                       |> Map.new()
  @front_exposure_types_int_to_ext_long_map Enum.zip(
                                              @front_exposure_types_int,
                                              @front_exposure_types_ext_long
                                            )
                                            |> Map.new()
  @front_exposure_types_ext_to_int_map Enum.zip(
                                         @front_exposure_types_ext,
                                         @front_exposure_types_int
                                       )
                                       |> Map.new()

  def map_front_exposure_type_int_to_ext(k) when is_atom(k),
    do: @front_exposure_types_int_to_ext_map[k]

  def map_front_exposure_type_int_to_ext(k) when is_binary(k),
    do: map_front_exposure_type_int_to_ext(String.to_existing_atom(k))

  def map_front_exposure_type_int_to_ext_long(k) when is_atom(k),
    do: @front_exposure_types_int_to_ext_long_map[k]

  def map_front_exposure_type_int_to_ext_long(k) when is_binary(k),
    do: map_front_exposure_type_int_to_ext_long(String.to_existing_atom(k))

  def map_front_exposure_type_ext_to_int(k) when is_binary(k),
    do: @front_exposure_types_ext_to_int_map[k]
  ...
  def map_style_type_int_to_ext(k) when is_binary(k),
    do: map_style_type_int_to_ext(String.to_existing_atom(k))
  ...
  @dining_room_types_int_to_ext_map Enum.zip(@dining_room_types_int, @dining_room_types_ext)
                                    |> Map.new()
  @dining_room_types_ext_to_int_map Enum.zip(@dining_room_types_ext, @dining_room_types_int)
                                    |> Map.new()

  def map_dining_room_type_int_to_ext(k) when is_atom(k), do: @dining_room_types_int_to_ext_map[k]

  def map_dining_room_type_int_to_ext(k) when is_binary(k),
    do: map_dining_room_type_int_to_ext(String.to_existing_atom(k))

  def map_dining_room_type_ext_to_int(k) when is_binary(k),
    do: @dining_room_types_ext_to_int_map[k]
  ...
  def map_fuel_type_int_to_ext(k) when is_binary(k),
    do: map_fuel_type_int_to_ext(String.to_existing_atom(k))
  ...
  def map_heating_type_int_to_ext(k) when is_binary(k),
    do: map_heating_type_int_to_ext(String.to_existing_atom(k))
  ...
  def map_sewage_type_int_to_ext(k) when is_binary(k),
    do: map_sewage_type_int_to_ext(String.to_existing_atom(k))
  ...
  def map_water_type_int_to_ext(k) when is_binary(k),
    do: map_water_type_int_to_ext(String.to_existing_atom(k))
  ...
  @sep_hw_heater_type_opts Enum.zip([" " | @sep_hw_heater_types_ext], [
                             nil | @sep_hw_heater_types_int
                           ])
  def sep_hw_heater_type_select_opts, do: @sep_hw_heater_type_opts

  @sep_hw_heater_types_int_to_ext_map Enum.zip(@sep_hw_heater_types_int, @sep_hw_heater_types_ext)
                                      |> Map.new()
  @sep_hw_heater_types_ext_to_int_map Enum.zip(@sep_hw_heater_types_ext, @sep_hw_heater_types_int)
                                      |> Map.new()

  def map_sep_hw_heater_type_int_to_ext(k) when is_atom(k),
    do: @sep_hw_heater_types_int_to_ext_map[k]

  def map_sep_hw_heater_type_int_to_ext(k) when is_binary(k),
    do: map_sep_hw_heater_type_int_to_ext(String.to_existing_atom(k))

  def map_sep_hw_heater_type_ext_to_int(k) when is_binary(k),
    do: @sep_hw_heater_types_ext_to_int_map[k]

  # Green Certification Types
  @green_cert_types_ext [
    "Energy Star",
    "HERS",
    "LEED",
    "LEED Gold",
  ...
  @green_cert_types_int_to_ext_map Enum.zip(@green_cert_types_int, @green_cert_types_ext)
                                   |> Map.new()
  @green_cert_types_ext_to_int_map Enum.zip(@green_cert_types_ext, @green_cert_types_int)
                                   |> Map.new()

  def map_green_cert_type_int_to_ext(k) when is_atom(k), do: @green_cert_types_int_to_ext_map[k]

  def map_green_cert_type_int_to_ext(k) when is_binary(k),
    do: map_green_cert_type_int_to_ext(String.to_existing_atom(k))
  ...
  def map_patio_type_int_to_ext(k) when is_binary(k),
    do: map_patio_type_int_to_ext(String.to_existing_atom(k))
  ...
  def map_porch_type_int_to_ext(k) when is_binary(k),
    do: map_porch_type_int_to_ext(String.to_existing_atom(k))
  ...
  def map_pool_type_int_to_ext(k) when is_binary(k),
    do: map_pool_type_int_to_ext(String.to_existing_atom(k))
  ...
  def map_deck_type_int_to_ext(k) when is_binary(k),
    do: map_deck_type_int_to_ext(String.to_existing_atom(k))
  ...
  def map_att_type_int_to_ext(k) when is_binary(k),
    do: map_att_type_int_to_ext(String.to_existing_atom(k))
  ...
  def map_eef_feature_int_to_ext(k) when is_binary(k),
    do: map_eef_feature_int_to_ext(String.to_existing_atom(k))
