defmodule Mpnetwork.CustomTimberLogger do
  ...
  def log(%Ecto.LogEntry{query: query, query_time: time_native} = entry, level) when is_function(query) do
    log(%Ecto.LogEntry{query: query.(entry), query_time: time_native}, level)
  end

  @spec log(Ecto.LogEntry.t, atom) :: Ecto.LogEntry.t
  def log(%Ecto.LogEntry{query: query, query_time: time_native} = _entry, level) when is_binary(query) do
    TimberLogger.log(%Ecto.LogEntry{query: transform_log_entry(query), query_time: time_native}, level)
  end
