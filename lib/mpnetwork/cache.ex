defmodule Mpnetwork.Cache do
  ...
  defp update_cache(%Cache{} = cache, attrs \\ %{}) do
    cache
    |> Cache.changeset(attrs)
    |> Repo.update(force: true) # forces timestamps to update
  end
  ...
  def fetch(cache_name, key, fallback, _opts) when is_function(fallback) do
    fallback = case fn_arity(fallback) do
      0 -> fn -> fallback.() end
      1 -> fn -> fallback.(key) end
    end
    case get_cache({cache_name, key}) do
      %Cache{value: value} -> {:ok, value}
      _                    ->
        fallback_val = fallback.()
        {:ok, %Cache{value: value}} = create_cache(%{key: {cache_name, key}, value: fallback_val, sha256_hash: hash(fallback_val)})
        {:loaded, value}
    end
  end
  ...
  def touch(cache_name, key, _opts) do
    case get_cache({cache_name, key}) do
      %Cache{} = cache -> update_cache(cache) # a no-op that just updates the timestamp
