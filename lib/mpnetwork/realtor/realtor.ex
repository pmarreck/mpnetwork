defmodule Mpnetwork.Realtor do
  ...
  def list_next_broker_oh_listings(nil, number, after_datetime) do
    now = after_datetime.()
    Repo.all(from l in Listing, where: l.next_broker_oh_start_at > ^now and l.draft == false, order_by: [asc: l.next_broker_oh_start_at], limit: ^number) |> Repo.preload([:broker, :user])
  end
